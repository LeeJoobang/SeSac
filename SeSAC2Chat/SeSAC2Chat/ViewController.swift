...
//
//        tableView.reloadData()
//        // MARK: scroll 되는 역할(자동), index로 처리했으나 인덱스로 처리할 경우 런타임 오류가 발생할 수 있음.
//        tableView.scrollToRow(at: IndexPath(item: dummy.count - 1, section: 0), at: .bottom, animated: false)
...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if data.userID == APIKey.userId { // userid가 같으면 내 셀에 표시하고 아니면 다른 셀에 표시해랏
    ...
    private func fetchChats() {
        ...
        AF.request(APIKey.url, method: .get, headers: header).responseDecodable(of: [Chat].self) { [weak self] response in
            ...
            case .success(let value):
                ...
                // MARK: 실시간으로 데이터를 받아 채팅방의 내용을 표시한다.
